...
public class ServerUDP extends Server {
    ...
    public void run() {
        while (!this.serverSocket.isClosed()) {
            try {
                new ClientConnection(ServerUDP.this).handleClient();
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
    ...
    private static class ClientConnection extends ServerClientConnection {
        ...
        public void handleClient() {
            while (true) {
                try {
                    String request = new String(this.fromClientPacket.getData(), this.fromClientPacket.getOffset(), this.fromClientPacket.getLength());
        ...
        protected void writeToClient(String msg) {
            try {
                ...
                DatagramPacket toClientPacket = new DatagramPacket(sendData, sendData.length, this.clientIpAddress, this.clientPort);
                server.serverSocket.send(toClientPacket);
            } catch (IOException ex) {
                handleConnectionException(ex);
            }
        }
        ...
        protected void playThisGame(Hangman game) {
            while (!game.won() && !game.lost()) {
                ...
                try {
                    ...
                    this.fromClientPacket = new DatagramPacket(receiveData, receiveData.length);
                    server.serverSocket.receive(this.fromClientPacket);
                    String input = new String(this.fromClientPacket.getData(), this.fromClientPacket.getOffset(), this.fromClientPacket.getLength());
